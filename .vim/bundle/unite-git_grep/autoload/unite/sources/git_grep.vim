...
function! s:source.change_candidates(args, context)"{{{
 if unite#sources#git_grep#check()
    ...
    return unite#sources#rgrep#grep(a:context.input)
 endif
endfunction"}}}
...
function! unite#sources#git_grep#grep(input)"{{{
 let l:result = unite#util#system('git grep -ni "' . a:input . '"')
...
function! unite#sources#git_grep#rgrep(input)"{{{
 let l:result = unite#util#system('grep -rni "' . a:input . '"')
 let l:matches = split(l:result, '\r\n\|\r\|\n')
  let l:entries = map(l:matches, '[v:val, split(v:val, ":")]')
  return map(l:entries,
    \ '{
    \   "word": v:val[0],
    \   "source": "vcs_grep/rgrep",
    \   "kind": "jump_list",
    \   "action__path": v:val[1][0],
    \   "action__line": v:val[1][1],
    \   "action__text": join(v:val[1][2:], ":"),
